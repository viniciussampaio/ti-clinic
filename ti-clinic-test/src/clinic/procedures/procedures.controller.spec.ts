...
describe('ProceduresController', () => {
  ...
  let service: jest.Mocked<ProceduresService>;
  ...
  it('create: delegates to service.create', async () => {
    ...
    await expect(controller.create({ name: 'Proc', price: 10 } as any)).resolves.toEqual({ id: 1 });
    expect(service.create).toHaveBeenCalledWith({ name: 'Proc', price: 10 });
  });
